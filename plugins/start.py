...
ADMIN = int(os.environ.get("ADMIN", 1280356202))
...
async def start(client, message):
    ...
    try:
        ...
        txt=f"""{wish} {message.from_user.first_name } \n\n
	I am file renamer bot, Please sent any telegram**Document Or Video** and enter new filename to rename it"""
        await message.reply_photo(photo=LAZY_PIC,
                                caption=txt,
                                reply_markup=InlineKeyboardMarkup(
                                      [[InlineKeyboardButton("🔺 Update Channel 🔺", url="https://t.me/TGBotsCode"),
                                     InlineKeyboardButton("👥 Support Group 👥", url="https://t.me/bjs_discussion")],
                                      [InlineKeyboardButton("Ⓜ️ Buy Premium", callback_data='/upgrade')],
                                      [InlineKeyboardButton("☕ Buy Me A Coffee ☕", url='https://paytm.me/m-HPBMt')]
                                      ]))
        return
    if id:
        if old == True:
...
async def send_doc(client, message):
    ...
    if update_channel:
        try:
            ...
            await client.send_message(log_channel,f"🦋 #renamer Bot Logs 🦋,\n\n**ID** : `{user_id}`\n**Name**: {message.from_user.first_name} {message.from_user.last_name}\n**User-Plan** : {user}\n\n ",
    ...
    try:
        ...
        await message.reply_text(text=f"Hello dear {message.from_user.first_name}  **we are currently working on this issue**\n\nPlease try to rename files from your another account.\nBecause this BOT can't rename file sent by some ids.\n\nIf you are an **ADMIN** Don't worry ! here we have a solution for you dear {message.from_user.first_name }.\n\nPlease use \n👉 `/addpremium your_other_userid` 👈 to use premium feautres\n\n",
                                  reply_markup=InlineKeyboardMarkup([
                                                                     [InlineKeyboardButton("🦋 Contact Admin", url='tg://user?id=1280356202')],
                                            [InlineKeyboardButton("🔺 Update Channel 🔺", url="https://t.me/TGBotsCode"),
                                     InlineKeyboardButton("👥 Support Group 👥", url="https://t.me/bjs_discussion")],
                                      [InlineKeyboardButton("Ⓜ️ Buy Premium", callback_data='/upgrade')],
                                      [InlineKeyboardButton("☕ Buy Me A Coffee ☕", url='https://paytm.me/m-HPBMt')]
        ...
        if value < file.file_size:
            ...
            if STRING:
                ...
                if pre_check == True:
                    ...
                    uploadlimit(message.from_user.id, 2684354560)
